...
export async function GET() {
  ...
  try {
    const pool = getPool();
    const apiKey = await getTranslationApiKey(pool, session.userId);
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    await setTranslationApiKey(pool, session.userId, apiKey);
...
export async function DELETE() {
  ...
  try {
    ...
    await clearTranslationApiKey(pool, session.userId);
    const cleanupScopes = resolveAiCleanupScopesForInputs({
      previous: {
        settings: uiSettings,
        aiApiKey,
